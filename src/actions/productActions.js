...
export const getAllProducts = (keyword="",currentPage=1,price=[0,25000],category,rating=0) => async (dispatch) => {
    try {
        ...
        let link = `${server}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${rating}`;

        if (category) {
          link = `${server}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&ratings[gte]=${rating}`;
        }
...
export const getProductDetails = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${server}/api/v1/products/${id}`,{withCredentials:true});
...
export const createAdminProduct=(productData)=>async(dispatch)=>{
    try {
        ...
        const {data}=await axios.post(`${server}/api/v1/admin/products/new`,productData,config);
...
export const updateAdminProduct=(id, productData)=>async(dispatch)=>{
    try {
        ...
        const {data}=await axios.put(`${server}/api/v1/admin/products/${id}`, productData, config);
...
export const deleteAdminProduct=(id)=>async(dispatch)=>{
    try {
        ...
        const {data}=await axios.delete(`${server}/api/v1/admin/products/${id}`,{withCredentials:true});
...
export const deleteAdminReview=(reviewId, productId)=>async(dispatch)=>{
    try {
        ...
        const {data}=await axios.delete(`${server}/api/v1/reviews?id=${reviewId}&productId=${productId}`,{withCredentials:true});
