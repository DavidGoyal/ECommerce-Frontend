...
export const getProductDetails = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${server}/api/v1/products/${id}`);
...
export const deleteAdminProduct=(id)=>async(dispatch)=>{
    try {
        ...
        const {data}=await axios.delete(`${server}/api/v1/admin/products/${id}`);
...
export const deleteAdminReview=(reviewId, productId)=>async(dispatch)=>{
    try {
        ...
        const {data}=await axios.delete(`${server}/api/v1/reviews?id=${reviewId}&productId=${productId}`);
